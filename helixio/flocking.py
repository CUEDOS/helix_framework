...
def single_torus_swarming(drone_id, swarm_pos_vel, my_pos_vel, time_step, max_accel):
    ...
    for i in range(Torus_points_num):
        redTorusPoints.append(
            np.array(
                [
                    a_ellipse * math.sin(i * 6.28 / Torus_points_num),
                    b_ellipse * math.cos(i * 6.28 / Torus_points_num),
                    -20,
                ]
            )
        )
    ...
    for i in range(Torus_points_num):
        range_to_point = np.linalg.norm(
            np.array(my_pos_vel.position_ned) - np.array(redTorusPoints[i])
        )
    ...
    v_laneCohesion = (
        (laneCohesionPositionError_magnitude - laneRadius)
        * laneCohesionPositionError
        / np.linalg.norm(laneCohesionPositionError)
    )

    if np.linalg.norm(v_laneCohesion) > limit_v_laneCohesion:
        v_laneCohesion = (
            v_laneCohesion * limit_v_laneCohesion / np.linalg.norm(v_laneCohesion)
        )
    ...
    v_rotation = (
        ...
        * np.cross(v_laneCohesion, targetDirection)
        / np.linalg.norm(np.cross(v_laneCohesion, targetDirection))
    )
