...
async def run():
    ...
    while 1:
        ...
        client.publish(mqtt_topic, str(time.time())+","+str(drone_telemetry.qx)+","+str(drone_telemetry.qy)+","+str(drone_telemetry.qz));
        
        #Checking frequency of the loop
        await asyncio.sleep(0.1-(time.time()-start_time)) # to make while 1 work at 10 Hz
