...
def visualize_path_following(**Input):
    ...
    for id in swarm_telem:
        drones.update(
            {
                id: [
                    Experiment(id, swarm_telem, JSON_file_dir),
                    [],
                    [],
                    [],
                    [],
        ...
        )  # {id: Experiment object, [record of x], [record of y], [record of z], [record of time], [record of v_migration], [record of v_rotation], [record of v_lane_cohesion], [reord of v_separation], [record of current_path], [record of current_index]}

    # Assigning prestart positions
    for id in swarm_telem:
        prestart_positon = drones[id][0].get_pre_start_positions(
            swarm_telem, drones[id][0].get_swarm_priorities(swarm_telem)
        )
        swarm_telem[id].position_ned = prestart_positon[id]

    for id in swarm_telem:
        drones[id][0].get_path_and_permission(
            drones[id][0].get_swarm_priorities(swarm_telem)
        )
    ...
    while t <= simulation_time:
        ...
        for id in swarm_telem:
            velocity_ned_yaw = drones[id][0].path_following(
                swarm_telem, max_speed, time_step, max_accel
            )  # object of class VelocityNedYaw
            drones[id][0].velocity_ned = np.array(
                [
        ...
        # Integrating
        for id in swarm_telem:
            swarm_telem[id].position_ned = (
                np.array(swarm_telem[id].position_ned) + drones[id][0].velocity_ned * dt
            )
    ...
    for id in drone_ids:  # to count ids in order
        ...
        print(
            "The least distance between dornes ",
            id,
            "and ",
            drones[id][0].min_distance[2],
            "=",
            drones[id][0].min_distance[0],
        )
        for i in range(simulation_steps):
            ...
            if CSV_order == "vertical":
                row = [
                    ...
                    1,
                    "Python_simulation",
                ]  # x, y , z, time (s), id, offboard mode status, type of experiment
                writer.writerow(row)

    print(
        "The smallest least distance is between dornes ",
        closest_drone_1,
        "and ",
        closest_drone_2,
        "=",
        samllest_min_distance,
    )
    ...
    fig = px.scatter_3d(
        ...
        y=Y_total,
        z=Z_total,
        animation_frame=Time_total,
        opacity=1,
        size=size,
        color=labels_total,
        size_max=max(size),
        color_discrete_sequence=fig_colors,
    )
    ...
    if show_vortices == True:
        ...
        for i in range(len(vortices)):
            fig.add_trace(
                go.Scatter3d(
                    x=[
                        vortex_centre[0] - np.sign(vortices[i][0]) * max_range,
                        vortex_centre[0] + np.sign(vortices[i][0]) * max_range,
                    ],
                    y=[
                        vortex_centre[1] - np.sign(vortices[i][1]) * max_range,
                        vortex_centre[1] + np.sign(vortices[i][1]) * max_range,
                    ],
                    z=[
                        -vortex_centre[2] + np.sign(vortices[i][2]) * max_range,
                        -vortex_centre[2] - np.sign(vortices[i][2]) * max_range,
                    ],
                    mode="lines",
                    line=dict(dash="dash"),
        ...
        for id in drone_ids:
            for swithced_position in drones[id][0].switched_positions:
                annotation_list.append(
                    dict(
                        x=swithced_position[0],
                        y=swithced_position[1],
                        z=-swithced_position[2],
                        text=id + " switched",
                        opacity=0.7,
            ...
            annotation_list.append(
                dict(
                    x=closest_drone_1_position[0],
                    y=closest_drone_1_position[1],
                    z=-closest_drone_1_position[2],
                    text=closest_drone_1 + " closest",
                    opacity=0.7,
            ...
            annotation_list.append(
                dict(
                    x=closest_drone_2_position[0],
                    y=closest_drone_2_position[1],
                    z=-closest_drone_2_position[2],
                    text=closest_drone_2 + " closest",
                    opacity=0.7,
                    font=dict(color="red", size=5),
                    arrowcolor="red",
                    arrowsize=1,
    ...
    fig.layout.scene.camera.projection.type = (
        "perspective"  # for orthographic projection set it to "orthographic"
    )
    fig.show()


visualize_path_following(
    drone_num=8,
    dt=0.1,
    output_CSV_file_dir="/home/m74744sa/Desktop/All_csvs/Python_sim.csv",
    JSON_file_dir="/home/m74744sa/Documents/helix_framework/helix_framework/experiments/helix_convergence.json",
    show_corridors=False,
    CSV_order="horizontal",
    show_annotations=False,
    cubic_space=True,
    show_vortices=True,
)
